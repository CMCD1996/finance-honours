...
E. Fama, with J. MacBeth, developed the Fama-MacBeth regression (\cite{fama1973risk}) to estimate factor pricings
The methodology is a two-stage estimation process for estimating factor loadings, and prices, for a given portfolio.
The first step requires determining each asset's $\beta$ exposures by regressing each of n asset excess returns against m proposed \ref{fb1}.
The second step determines the risk premium (factor pricing) for each asset by regressing all asset excess returns for each T period against previously estimated $\beta$s (\ref{fb2}).
\begin{align*}
	ER_{n,t} &= \alpha_{n} + \sum_{f=1}^{F} \beta_{n,F_{f}}F_{f,t} + \epsilon_{n,t} \numberthis \label{fb1} \space \forall n\epsilon\{1,...,N\}\\
\end{align*}
Where
\begin{itemize}
	\item 
\end{itemize}
\textbf{The matrix notation for these formulations follows:}
\begin{align*}
	H_{\mu} &=1 \numberthis \label{hm}\\
...
\end{align*}
Subsequently, this research essay uses the Hedge Portfolio Mean (\ref{hpm})), Capital Asset Pricing Model (\ref{capm}), 

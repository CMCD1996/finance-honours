...
\begin{titlepage}
	...
	{\huge\bfseries Direct Hedge Portfolio Excess Return Maximisation using Deep Neural Networks}\\[0.4cm] % Title of your document
...
\newpage
\section*{Abstract}
The publication by Gu et al. (\citeyear{eapvml}) illustrates the improvement to empirical asset pricing via machine learning.
...
The data used is factor portfolio dataset from Hou et al. (\citeyear{hou2020replicating}), developed by Jensen et al. (\citeyear{jensen2021there}) with
training, testing, and validation sets of 532218, 294581, and 531461 global equity firm-year observations across 160 features,
chronological divided between 1961-1989, 1990-1999, and 2000-2020, respectively.
The maximisation function is a non-convex function seeking to maximise hedge portfolio returns, with hedge portfolio weights determined by a monotonic ranking function mapping.
...
After model training and evaluation, the trained models are accruate, but do not outperform models trained using conventional loss minimisation functions.
However, limitations render the research question inconclusive at this time.
\newpage
\tableofcontents
\listoffigures
\listoftables
...
\input{{/Users/connor/Google Drive/Documents/University/Courses/2020-21/Finance 788/finance-honours/results/tex/cmcd398-finance-honours-introduction}.tex}
\newpage
...
Figure \ref{fig:neural-network-configuration} illustrates the designated neural network configuration.\footnote{Optimal layer density as per Gu et al. (\cite{eapvml}) with three hidden layers (The dropout is a processing layer to prevent overfitting).}
...
The use of dense layers deeply connects two layers where each node receives an input from the previous layer.
...
Table \ref{main-findings} summarises main findings across model training, validation, individual return predictability, performance measures, and hedge portfolio comparisons.
Gu et al. (\citeyear{eapvml}) benchmarks analysis \footnote{Discrepancies in data and methodologies exist, raised appropriately}.
\input{{/Users/connor/Google Drive/Documents/University/Courses/2020-21/Finance 788/finance-honours/results/tex/cmcd398-finance-honours-main-findings}.tex}
\singlespacing
\begin{enumerate}
	\item $\theta_{(Final)}$ is the final training loss after training.
	\item $\lambda_{(Final)}$ is the final validation loss after validation.
	\item OMLER is the coefficient when regressing actual lead one month returns on predicted.
	\item $\mu_{(HP)}$ is the hedge portfolio mean across all months.
	\item $Sharpe_{HP}$ is the sharpe ratio for the hedge portfolio
	\item $Treynor_{HP}$ is the treynor ratio for the hedge portfolio
	\item $\beta_{HP}$ is systematic market risk for the hedge portolio
	...
	\item Adjusted $R^{2}$ is explained variation when regressing actual one month lead excess returns on predicted returns.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{custom-hp-learning-curve.png}
	\caption{Hedge Portfolio Learning Curves}
	\label{fig:hp-lc}
\end{figure}
\begin{figure}[H]
	\centering
	\subfloat[In-Built Mean Square Error Learning curve\label{fig-la-mse}]{%
		\includegraphics[width=0.8\textwidth]{mean-squared-error-learning-curve.png}
	}
	\vfill
	\subfloat[Custom MSE\label{fig-lb-cmse}]{%
		\includegraphics[width=0.8\textwidth]{custom-mse-learning-curve.png}
	}
...
Table \ref{return-predictability} regresses realised one month lead excess returns, on the predicted returns, for every loss function.
The F Statistic on all tests is highly statistically significant at the one percent level,
verifying realised returns are derivative of predictions.
The out-of-sample adjusted $R^{2}$ values are 0.012, 0.042, and 0.001 for $MSE$($\hat{y}$,y), $MSE_{Custom}$($\hat{y}$,y), and $Hedge Portfolio $($\hat{y})$ respectively.
Gu et al. (\citeyear{eapvml}) has a range of out-of-sample from 0.33\% (0.0033) to 0.40\% (0.004) for non-linear predictions.
Our are higher, but may be from using a more powerful algorithm (neural networks) and one-sixth the number of factors.
The coefficients of $MSE$($\hat{y}$,y), $MSE_{Custom}$($\hat{y}$,y), and $Hedge Portfolio $($\hat{y})$ highly statistical significant at the one percent level, 0.129, 0.238, and -1.319, respectively.
The negative coefficient is not unexpected given the objective function seeks to maximise predicted hedge portfolio returns opposed minimising the loss between actual and realised.
Subsequently, it is feasible to use maximisation strategies in neural networks, confirming our first hypothesis.
Ultimately, the adoption of maximisation strategies lies in their economic significance.
...
Figure \ref{performance-metrics} displays the mean, sharpe ratio, and treynor ratio from the predicted hedge portfolios per loss function.
...
These raise concern given they exceed observations from prior literature.
However, there are some hypotheses.
Section \ref{jensen-fpc} describe the method Jensen et al. (\citeyear{jensen2021there}) in constructing
factor portfolios. In summary, they sort stocks into terciles in the 20th percentile on market capitalisation or higher first,
then evenly distribute all the stock below the 20th percentile on market capitalisation evenly into the three terciles using similar sorting characteristics.
Following, each tercile has it's capped value weight return calculated, ensuring tiny stocks have tiny weight, and large stocks don't dominate the portfolio.
The predicting of construction of hedge portfolios should reflect this methodology.
However, equities below the 20th percentile were omitted, as mentioned in section \ref{data}.\footnote{Discovered very late in analysis, well after datapipeline were finalised infrastructure}
Therefore, large stocks dominate the portfolio, raising the hedge portfolio mean.
Preliminary analysis used deciles, recording 0.05 and 0.1 for $MSE$($\hat{y}$,y), and $MSE_{Custom}$($\hat{y}$,y), respectively.
The change to deciles reduced the hedge portfolio means to the aforementioned figures.
For context, the mean excess return on realised one month lead excess returns is 0.0007 (0.7\%).
The treynor ratio indicates the excess return on systematic risk is consistent across the three strategies (~-0.30).
...
\subsubsection{Hedge Portfolio Mean, Sharpe, and Treynor}
...
The market risk premia factor is negative in all models across loss functions.
The out-of-sample period includes both the Dotcom crash and Financial Crisis.
...
\input{{/Users/connor/Google Drive/Documents/University/Courses/2020-21/Finance 788/finance-honours/results/tex/cmcd398-finance-honours-mean\_squared\_error}.tex}
\newpage
\input{{/Users/connor/Google Drive/Documents/University/Courses/2020-21/Finance 788/finance-honours/results/tex/cmcd398-finance-honours-custom\_mse}.tex}
\newpage
\input{{/Users/connor/Google Drive/Documents/University/Courses/2020-21/Finance 788/finance-honours/results/tex/cmcd398-finance-honours-custom\_hp}.tex}
\newpage
\section{Research Implications} \label{research-implications}
\subsection{Contributions}
\subsubsection{Maximisation Strategies}
Maximisation strategies are feasible from a technical perspective.
Neural net architecture, and automatic differentiation in Keras Backend,
enable the construction of custom optimisation functions
in consideration of objectives outside standard loss minimisation.
You can consider the maximisation of hedge portfolios,
However, the extent to their economic significance remains unanswered.
This is a partial, novel contribution to the literature.
\subsubsection{Use of Factor Portfolio Dataset}
A minor contribution is validating the use of factor portfolio dataset by Jensen et al. (\citeyear{jensen2021there}) for empirical asset pricing analysis with machine learning techniques.
The dataset is of sufficient size, quality, and granularity for this form of analysis.
...
Other optimisation techniques exist which may add value e.g., reinforcement learning, dynamic programming etc.
...
\input{{/Users/connor/Google Drive/Documents/University/Courses/2020-21/Finance 788/finance-honours/results/tex/cmcd398-finance-honours-conclusion}.tex}
\newpage
\printbibliography
\newpage
...
\subsubsection{Description \& Sources}
...
\subsubsection{Factor Portfolio Construction}\label{jensen-fpc}
The below enumeration describes verbatim one month holding period return within each country by Jensen et al. (\cite{jensen2021there}).
\begin{enumerate}
	\item In each country and month, we sort stocks into characteristic terciles (top, middle, and bottom
	      with breakpoints based on non-micro stocks in that country. Specifically, we
	      start with all non-micro stocks in a country (i.e., larger than NYSE 20th percentile)
	      and sort them into three groups of equal numbers of stocks based on the characteristic,
	      say book-to-market. Then we distribute the micro-cap stocks into the three groups
	      based on the same characteristic breakpoints. This process ensures that the non-micro
	      stocks are distributed equally among across portfolios, creating more tradable portfolios.
	\item For each tercile, we compute its capped value weight" return, meaning that we weight
	...
	\item The factor is then defined as the high-tercile return minus the low-tercile return, corre-
	      ...
	      expected return.
	\item For a factor return to be non-missing, we require that it has at least 5 stocks in each of

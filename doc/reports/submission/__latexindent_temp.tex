...
Figure \ref{fig:neural-network-configuration} illustrates the required configuration for evaluating excess returns.\footnote{Configuration uses both Newtonian and Matrix Notation}
$N(x,y)$ represents the yth node in xth layer. .
\textbf{z} = \text{$\sum_{a = 1}^{189} w_{(n,a,b)}x_{(n,a,b)}$ ($=\textbf{w}^{T} \cdot \textbf{x}$) represents the dot product of all outputs (x) and connection weights (w) from the nth layer, between the nodes a and b)in layers n and n+1, respectively.
...
\begin{figure}[H]
    \includegraphics[width=1.5\textwidth]{mean-square-error-tensorflow.png}
    \caption{Performance: In-Built Mean Square Error}
    \label{fig:mse-tf-performance}
\end{figure}
\begin{figure}[H]
    \includegraphics[width=1.5\textwidth]{mean-square-error.png}

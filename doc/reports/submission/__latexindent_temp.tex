...
\addbibresource{finance-honours.bib}
...
where
\begin{itemize}
	\item $R_{i,t} - R_{f,t}$: Portfolio Excess Return for a given portfolio and time
	\item $\alpha_{i,t}$: 
	\item $\beta_{t}^{1}$: Market Risk Factor
	\item \item $\beta_{t}^{1}$: Size Pricing Factor
	\item \item $\beta_{t}^{2}$: Market Risk (sensitivity)
	\item \item $\beta_{t}^{3}$: Market Risk (sensitivity)
	\item \item $\beta_{t}^{4}$: Market Risk (sensitivity)
	\item $(R_{M,t}-R_{f,t})$: Market Risk Premium
	\item $SMB_{t}$: Size Premium (small minus big)
	\item $HML_{t}$: Value Premium (high minus low)
	\item $ $
	\item $ $
\end{itemize}

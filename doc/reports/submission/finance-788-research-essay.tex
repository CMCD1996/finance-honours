...
\singlespacing
    ...
    \item $\alpha_{(HP,FF3)}$, $\alpha_{(HP,FF3)}$, $\alpha_{(HP,FF4)}$, and $\alpha_{(HP,FF5)}$ are the hedge portfolio alphas generated in their respective asset pricing models.
...
Ultimately, -0.016803, -0.714680, and -0.283188 for hedge portfolio mean and sharpe ratio shows the hedge portfolio maximisation strategy does not outperform traditional loss minimisation regardless of accuracy.
...
Figures \ref{mse-tf-apm}, \ref{mse-apm-cmse}, and \ref{hp-apm-hp} show estimations for hedge portfolio alpha per month, for each standard factor pricing model, using an optimisation strategy, 
...
\newpage
\subsubsection{Asset Pricing: $MSE_{Custom}(\hat{y},y)$}
...
\newpage
\subsubsection{Hedge Portfolio ($\hat{y})$}
...
\subsection{Maximisation Strategies}

...
Please note the use of Hadamard exponentiation ($x^{\circ n}$).
\begin{align}
	f(y, X^{T} \hat{\theta}) &= \frac{\vec{1}}{\vec{1}^{T}\vec{1}} (y - X^{T}\hat{\theta})^{\circ 2} \label{mself}\\
	\frac{\partial f(y, X^{T} \hat{\theta})}{ \partial \hat{\theta}} &= \frac{\vec{1}}{\vec{1}^{T}\vec{1}} -2(y-X{T} \hat{\theta})^{\circ 1} \label{pdmself}\\
\end{align}
\subsubsection{Hedge Portfolio}
Hedge portfolios rely on monotonic ranking functions for optimisation as the monotonic nature preserves or reverses a given ordered set.
...
Long (L) or Short (S) sets include exces returns conditioned on the associated monotonic ranking given a threshold, bound by the cardinality of the excess return vector ($|y|$).
...
\end{align}
Figure \ref{fig:mrf}) illustrates a basic example
\begin{figure}[H]
    \centering 
    ...
    \caption{Visual Presentation of Equally Weighted Monotonic Ranking Function}
    \label{fig:mrf}
\end{figure}
...
\end{align}
$\argmax_{\hat{\theta}}$ uses partial derivatives with respect to $\hat{\theta}$ to find the optimum scenario.
\begin{align}
...
\subsubsection{Hedge Portfolio Mean}
\subsubsection{Hedge Portfolio Alphas (CAPM, FF3, FF5)}

...
Asset pricing theory is a cornerstone in prior
Capital Asset Pricing Model (CAPM) use persists, regardless of the invalidation from identifiable shortcomings in market proxies and empirical failings invalidating use (\cite{fama2004capital}).
...
Subsequently, the complete dataset has 1.112 billion data points. \textbf{One month lead excess returns}\footnote{One month lead excess returns (ret\_exc\_lead1m is RET-T30RET)/21 from CRSP, extracted using WRDS. T30 is the treasury yield rate. 
If T30RET is unavailable,
is used instead RF. If the return is a daily return rather than a monthly return, (RET - T30RET) is divided by 1 rather than 21.} is the designated target variable for prediction as will inform the construction of hedge portfolios to
assess relative performance between loss functions.
...
\begin{enumerate}
	...
	\item Standard monthly sorts of predicted one month lead excess returns form standard tercile hedge portfolios returns per month.
...
Table \ref{main-findings} summarises main findings
...
\begin{figure}[H]
	\centering
    \subfloat[In-Built Mean Square Error Learning curve\label{fig-lc-mse}]{%
      \includegraphics[width=0.8\textwidth]{mean-squared-error-learning-curve.png}
    }
    \vfill
    \subfloat[Custom MSE\label{fig-lc-cmse}]{%
  ...
  \begin{figure}[H]
	...
	\label{fig:hp-lc}
  \end{figure}
Figure \ref{fig:mse-learning-curves} and \ref{fig:hp-lc} visualise learning curves for mean squared error variants, and hedge portfolio loss functions, respectively.
Learning curves serve as diagnostic tools for underfitting, or overfitting.\footnote{Overfitting: Overlearns training data, generalises new data poorly. Underfitting: Inadequate training leading to poor predictions.}
\textbf{Elaborate all this curves show the dataset is representative, suitable for ML contributions}
...
\footnote{Generate using a Custom \href{https://neptune.ai/}{Neptune.ai} callback during model fitting.}
\subsection{Return Predictability}
Table \ref{predicability-actual-returns} regress one month lead excess returns, on the predicted returns, for every loss function.
...
However, the hedge portfolio loss function optimises predicted hedge portfolios.
...
\newpage
\subsubsection{Mean Square Error (Custom)}

...
Section \ref{technical} elaborates further these cloud centric computational approaches.
...
\begin{figure}[H]
	...
	\draw [arrow] (hidden1) -- node[anchor=west] {\textbf{y} = \text{$\sum_{b = 1}^{32} w_{(1,b,c)}g_{(1,b,c)}$} \text{$\forall$ c}} (hidden2);

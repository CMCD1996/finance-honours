...
\begin{figure}[H]
    ...
    \label{fig:mrf}
\end{figure}
First, we explore a ranking function with weights directly proportional to returns (equation \ref{w1}).
\begin{align}
	R(y_{i,t}) &= W_{i,t} \hat{y}_{i,t}\\
	\hat{y} &= X^{T} \hat{\theta} + \epsilon\\
	W&:= \frac{\hat{y}(\hat{\theta})}{\textbf{1}\hat{y}(\hat{\theta})} \label{w1}\\
	\argmax_{\hat{\theta}} &: \frac{\hat{y}}{\textbf{1}\hat{y}} \hat{y}\\
	\argmax_{\hat{\theta}} &: \frac{X^{T} \hat{\theta}}{\vec{\textbf{1}}X^{T} \hat{\theta}} X^{T} \hat{\theta}
\end{align}
